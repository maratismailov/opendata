<script>
    ...
    const generate_survey = async (id) => {
        ...
        const pre_objects = await fetch(server).then((response) => response.json());
        const pre_survey = await fetch(server2).then((response) => response.json());
        ...
        const mbitles = await fetch(mbtiles_url).then((response) => response.blob().then((blob) => save_mbtiles(blob, survey_name)));
